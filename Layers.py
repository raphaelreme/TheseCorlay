...
class Layers:
    ...
    def __init__(self, output_size, input_size, function = nd.sigmoid, ctx = mx.cpu(0), fixed = False):
        ...
        weights = nd.random.normal(loc = 0, scale = 1, shape = (outut_size, input_size), ctx = self.ctx))
        bias = nd.random.normal(loc = 0, scale = 1, shape = output_size, ctx = self.ctx)
        ...
        try:
            if len(fixed.shape) == 1:
                if fixed.shape[0] = input_size + 1:
                    self.bias_fixed = nd.full(self.bias.shape, fixed[0], ctx = self.ctx)
                    self.weights_fixed = nd.dot(nd.full((output_size, 1), 1), fixed[1:].reshape((1, input_size))).copyto(self.ctx)
                elif fixed.shape[0] = output_size:
                    self.bias_fixed = fixed.copyto(self.ctx)
                    self.weights_fixed = nd.dot(fixed.reshape((output_size, 1)), nd.full((1, input_size))).copyto(self.ctx)
                else:
                    raise ValueError(message)
            elif fixed.shape = (output_size, input_size + 1):
